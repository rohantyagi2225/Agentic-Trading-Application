...
export default function SignalStream({ initialSymbol = "AAPL", compact = false, hideSelector = false }) {
  const [symbol, setSymbol] = useState(initialSymbol);
  const { messages, status, reconnect } = useSignalStream(symbol, 12);
    ...
    <div className="flex flex-col h-full gap-2">
      <div className="flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          {canReconnect && (
            <button onClick={reconnect} className="text-[10px] font-mono text-zinc-500 hover:text-cyan-400 underline underline-offset-2 transition-colors">
              retry
            </button>
          )}
        </div>
        <div className="flex items-center gap-2">
          {messages.length > 0 && <span className="text-[10px] font-mono text-zinc-600">latest {messages.length}</span>}
          {!hideSelector && (
            <select
              value={symbol}
      ...
      <div className={`flex-1 overflow-y-auto min-h-0 space-y-1 font-mono text-xs ${compact ? "max-h-72" : ""}`}>
        {messages.length === 0 ? (
          ...
          messages.map((msg, i) => (
            <div
              key={`${msg._ts}-${i}`}
              className={`flex items-center gap-2 px-2 py-1.5 rounded transition-colors ${i === 0 ? "bg-zinc-800/80 border border-zinc-700/50" : "hover:bg-zinc-800/30"}`}
            >
              ...
              {msg.confidence != null && <span className="text-zinc-500 tabular-nums ml-auto">{(msg.confidence * 100).toFixed(0)}%</span>}
